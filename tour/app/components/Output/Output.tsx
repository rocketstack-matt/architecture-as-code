...
const SchemaError = ({ schemaPath, errorMessage }: { schemaPath: string, errorMessage?: string }) => {
  ...
  if (errorMessage && schemaPath.includes('calmError')) {
    // Determine what kind of error we have to give more specific help
    let helpItems = [];
    
    if (errorMessage.includes('$schema')) {
      helpItems.push(<ListItem key="schema">Make sure your CALM document includes the $schema reference</ListItem>);
    }
    
    if (errorMessage.includes('node-type')) {
      helpItems.push(<ListItem key="nodetype">The node-type must be 'service' for this lesson</ListItem>);
    }
    
    if (errorMessage.includes('required properties')) {
      helpItems.push(<ListItem key="props">Ensure your node has unique-id, node-type, name, and description properties</ListItem>);
    }
    
    // Default help items if nothing specific matched
    if (helpItems.length === 0) {
      helpItems = [
        <ListItem key="structure">Make sure your CALM document follows the required structure</ListItem>,
        <ListItem key="schema">Keep the $schema reference in your document</ListItem>,
        <ListItem key="properties">Ensure all required properties are included for your service node</ListItem>
      ];
    }
