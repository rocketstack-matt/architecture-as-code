...
export async function validateCode(
  ...
  try {
    ...
    // Always start as neutral until validation proves otherwise
    let validationStatus: "valid" | "invalid" | "neutral" = "neutral";
    
    const totalTestCases = testCases.length;

    // For all lessons, validate the schema directly
    const schemaValidation = await hyperjumpValidate(schemaCode, schemaCode);
    if (!schemaValidation.valid) {
      validationStatus = "invalid";
      dispatchOutput({ 
        type: "invalidSchema", 
        payload: { errors: schemaValidation }
      });
      return;
    } else {
    ...
    // Process test cases if we have any
      ...
      if (validationResult.valid !== dataTestCase.expected) {
        testCaseResults.push({
          ...
          errors: validationResult.errors ? 
            validationResult.errors.map((e: any) => e.error || e.instanceLocation).join(', ') : "",
